...
//	It also returns both current [0] and avg[1] over the last three years
...
//		try {
//			equity = Long.valueOf(formatSciNotation(stock.getProfile().get("Total shareholders equity0")));
//			debt = Long.valueOf(formatSciNotation(stock.getProfile().get("Long-term debt0")));
//			cf = Long.valueOf(formatSciNotation(stock.getProfile().get("Operating Cash Flow0")));
//			capEx = Long.valueOf(Evaluator.formatSciNotation(stock.getProfile().get("Capital Expenditure0")));
//		} catch (NumberFormatException e) {
//			equity = Double.valueOf(formatSciNotation(stock.getProfile().get("Total shareholders equity0")))
//					.longValue();
//			debt = Double.valueOf(formatSciNotation(stock.getProfile().get("Long-term debt0"))).longValue();
//			cf = Double.valueOf(formatSciNotation(stock.getProfile().get("Operating Cash Flow0")))
//					.longValue();
//			capEx = Double.valueOf(formatSciNotation(stock.getProfile().get("Capital Expenditure0")))
//					.longValue();
//		}
//REMOVE THIS
...
//		System.out.println("store manuals run");
	...
	public static void changedGrowthChoice(int choice, CompanyController controller) {
		...
		for (Stock stock : CompanyController.getStocks()) {
			double totalFv;
			System.out.println(getTotalFutureValue(stock, "", controller.getReturnOnInvestment()) + " prior");
			if (choice == 2) {
				totalFv = getTotalFutureValue(stock, "croic", controller.getReturnOnInvestment());
			} else {
				totalFv = stock.getFutureValue();
			}

			double sharesOut = Long.valueOf(stock.getSharesOut());
			System.out.println(totalFv);
			...
			System.out.println(totalOverShares.toPlainString());
