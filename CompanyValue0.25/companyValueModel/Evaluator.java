...
//	expressed in an int to be rendered as a percentage.  Negative percentage indicates more spent on capex than than div and bb
	private static Integer[] calculateCapEff(long[] capex, long[] div, long[] bb, Stock stock) {
		long[] cap = new long[5];
...
//				this if block catches likely erroneous data from the api
	...
	private static String[] checkAssetsOrCapital(String key) {
	...
	private static long[] tAssets(String[] keys, Stock stock) {
	...
	private static long[] iCapital(String[] keys, Stock stock) {
...
//	calculates and fills the dcf multipliers array
	private static BigDecimal[] calculateDCFMultipliers(BigDecimal divisor) {
...
//	this method is tested by the test for getTotalDCF() method
	private static long calculateAndTotalDCF(BigDecimal[] dcfMultipliers, long[] fcfForCalculation,
...
//	Returns the average of the last three years fcf with current fcf double-weighted
	...
	static BigDecimal getFairValue(Stock stock) {
		...
		try {
			...
			System.out.println("Shares data missing");
