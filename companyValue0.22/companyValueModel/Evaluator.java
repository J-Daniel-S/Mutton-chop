...
//	compares the current profit margin to the average over the last five years (inclusive)
	static int avgProfitMargin(Long[] margins) throws NumberFormatException {
	...
	static long getCashOnHand(Stock stock) {
		long coh;
		return coh = Long.valueOf(stock.getProfile().get("Cash and cash equivalents"));
	}
...
//	Returns the average of the last three years fcf with current fcf double-weighted
	...
	static BigDecimal getFairValue(Stock stock) {
		...
		BigDecimal sharesOutstanding = BigDecimal.valueOf(Long.valueOf(getSharesOutstanding(stock))).setScale(7,
				RoundingMode.HALF_EVEN);
