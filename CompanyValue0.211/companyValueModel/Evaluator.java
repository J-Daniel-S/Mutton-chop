...
//	This method gets sharesOut from profile given by fmpwebreaderfilter and returns it as a String
	...
	static long getTotalDcf(Stock stock, String croic, String percent) {
		...
		if (stock.getProfile() == null) {
			...
			fcfGrowth = Evaluator.getFCFChangePercentRecentAndAverageArr(stock)[1];
		}
		
		if (croic == "croic") {
			fcfGrowth = Double.valueOf(stock.getCroic());
		}
...
//		This block prevents calculation in the case that the growth is too inconsistent or crazy to be useful for
...
//	returns the shareholder equity from the most recent year from the fmpweb profile
	static long getCurrentEquity(Stock stock) {
		...
		try {
			...
			equity = Long.valueOf(stock.getEquity());
		}
...
//	It will also be called by the program every time a new annual report is released. STILL WORKING ON DATE CHECK
	static void storeDetails(Stock stock) {
		...
		stock.setFairValue(Evaluator.getFairValue(stock).multiply(BigDecimal.valueOf(0.9725)).setScale(4, RoundingMode.HALF_EVEN));
...
//	I am still writing this!!!!!!!!!!!!!!!!!!!!!!!!!!!! - I may not need it
...
//			
//			BigDecimal totalOverShares = BigDecimal.valueOf(totalFv).divide(BigDecimal.valueOf(sharesOut), mc);
//			
...
//		this program overcalculates based on my google doc tests by a small percentage and this helps mitigate that
		fairValue = fairValue.multiply(BigDecimal.valueOf(0.9725).setScale(4, RoundingMode.HALF_EVEN));
...
//		System.out.println("store manuals run");
	...
	/*
	 * I do not like this implementation very much but what I did was passed either a 1 or a two back from the mainFrame
	 * to indicate whether I wanted to calculate the fair value based on 1) past growth rate or 2) croic.
	 * 
	 * I then passed string into the total dcf method that either contained "croic" in the case of two or was empty
	 * in the case of 1.  It works but I don't like it.  I may refactor later but I think I should leave well enough
	 * alone.
	 */
	public static void changedGrowthChoice(int choice, CompanyController controller) {
		MathContext mc = new MathContext(6, RoundingMode.HALF_EVEN);
		for (Stock stock: CompanyController.getStocks()) {
			double totalFv;
			if (choice == 2) {
				totalFv = Evaluator.getTotalFutureValue(stock, "croic", controller.getReturnOnInvestment());
			} else {
				totalFv = Evaluator.getTotalFutureValue(stock,"" , controller.getReturnOnInvestment());
			}
			
			double sharesOut = Long.valueOf(stock.getSharesOut());
...
//			roughly corrects the discrepancy between google sheets and this program
			totalOverShares = totalOverShares.multiply(BigDecimal.valueOf(0.9725).setScale(4, RoundingMode.HALF_EVEN));
	...
	public static void changedReturn(CompanyController controller) {
		MathContext mc = new MathContext(6, RoundingMode.HALF_EVEN);
		for (Stock stock : CompanyController.getStocks()) {
			double totalFv = Evaluator.getTotalFutureValue(stock, controller.getReturnOnInvestment());
			double sharesOut = Long.valueOf(stock.getSharesOut());
