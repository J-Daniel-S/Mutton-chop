...
//						everything below here has years worth of data on the api requiring handling.  Dividend payments and dividends per share are covered by the last one
	...
	private String[] formatAddedNumbers(String[] keyValue, String ticker) {
		if (keyValue[0].equals("yearLow") || keyValue[0].equals("yearHigh") || keyValue[0].equals("priceAvg200")
			...
			keyValue[1] = Evaluator.roundFloats(keyValue[1], ticker);
		}
		if (keyValue[0].contains("marketCap") || keyValue[0].equals("Revenue") || keyValue[0].equals("Net Income")
			...
			keyValue[1] = Evaluator.formatSciNotation(keyValue[1], ticker);
...
//		filters out the unchanged keys and revenue growth (r growth causes problems when building arrays later)
	...
	public HashMap<String, String> updatePriceData(Stock stock) throws UnsupportedEncodingException, IOException {
		...
		update.put(updateEV(stock)[0], updateEV(stock)[1]);
		update.put(updateNumOfShares(stock)[0], updateNumOfShares(stock)[1]);
		return update;
	}

	private String[] updateEV(Stock stock) throws UnsupportedEncodingException, IOException {
		String[] ev = new String[2];
		URL url = new URL("https://financialmodelingprep.com/api/v3/enterprise-value/" + stock.getTicker());
		boolean found = false;
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(url.openStream(), "UTF-8"))) {
			for (String line; (line = reader.readLine()) != null;) {
				if (line.contains("Enterprise Value") && found == false) {
					line = line.replace("\"", "").replace(",", "");
					String[] keyValue = line.split(":");
					keyValue[0] = keyValue[0].trim();
					keyValue[1] = keyValue[1].trim();
	...
	private String[] updateNumOfShares(Stock stock) throws UnsupportedEncodingException, IOException {
		String[] num = new String[2];
		URL url = new URL("https://financialmodelingprep.com/api/v3/enterprise-value/" + stock.getTicker());
		boolean shares = false;
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(url.openStream(), "UTF-8"))) {
			for (String line; (line = reader.readLine()) != null;) {
				if (line.contains("Number of Shares") && shares == false) {
					line = line.replace("\"", "").replace(",", "");
					String[] keyValue = line.split(":");
					keyValue[0] = keyValue[0].trim();
					keyValue[1] = keyValue[1].trim();
