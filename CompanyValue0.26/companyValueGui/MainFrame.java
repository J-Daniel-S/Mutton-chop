					...
					try {
						...
						if (name != null) {
							...
							if (action == JOptionPane.YES_OPTION) {
								...
								TableController.setData(CompanyController.getStocks(), infoTablePanel,
										finTablePanel, retTablePanel);
								TableController.refresh(infoTablePanel, finTablePanel, retTablePanel);
								MainFrame.this
										.addToTextPanel(CompanyController.getStocks()
												.get(CompanyController.getStocks().size() - 1).getDescription());
								if (CompanyController.getStocks().get(
										CompanyController.getStocks().size() - 1).getDescription()
												== null) {
