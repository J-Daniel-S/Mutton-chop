...
public class Searcher {
	...
	static Stock searchByTicker(StocksList stocks, String search) {
		...
		Optional<Stock> findStock = stocks.getStocks().stream()
				.filter(stk -> stk.getTicker().contains(search))
				.findAny();
	...
	static Stock searchByName(StocksList stocks, String search) {
		...
		Optional<Stock> findStock = stocks.getStocks().stream()
				.filter(stk -> stk.getName().contains(search))
				.findAny();
	...
	static List<Stock> searchByIndustry(StocksList stocks, String search) {
		List<Stock> findStocks = stocks.getStocks().stream()
				.filter(stk -> stk.getIndustry().contains(search))
	...
	static List<Stock> searchBySector(StocksList stocks, String search) {
		List<Stock> findStocks = stocks.getStocks().stream()
				.filter(stk -> stk.getSector().contains(search))
				.collect(Collectors.toList());
		return findStocks;
	}
}
